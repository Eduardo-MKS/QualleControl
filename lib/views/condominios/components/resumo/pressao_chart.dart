...
class PressaoChart extends StatelessWidget {
  final List<Map<String, dynamic>> historicoData;

  const PressaoChart({Key? key, required this.historicoData}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (filteredData.isEmpty) {
      return const SizedBox(
        height: 200,
        child: Center(child: Text('Sem dados de pressão nas últimas 24 horas')),
      );
    }
    ...
    final spots =
        filteredData.asMap().entries.map((entry) {
          ...
          final pressaoSaida = (data['pressao_saida'] as num).toDouble();
        ...
        LineChartData(
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              isCurved: true,
              curveSmoothness: 0.2,
              color: const Color.fromARGB(255, 67, 161, 70),
              barWidth: 3,
              isStrokeCapRound: true,
              dotData: FlDotData(show: false),
              belowBarData: BarAreaData(
                show: true,
                color: const Color.fromARGB(255, 88, 191, 92).withOpacity(0.2),
              ),
            ),
          ],
        ),
